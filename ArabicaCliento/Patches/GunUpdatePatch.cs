...
public class GunUpdatePatch
{
    ...
    public static NetCoordinates Patch(NetCoordinates originalCoords)
    {
        ...
        if (!ArabicaConfig.RangedAimbotEnabled) return originalCoords;
        ...
        if (ArabicaConfig.UsePrediction)
        {
            try 
            {
                ...
                if (gunUid != null && gunUid.Value.IsValid())
                {
                    ...
                    float bulletSpeed = ArabicaConfig.DefaultProjectileSpeed;
                    ...
                    // ЛОГИРОВАНИЕ
                    if (isHitscan)
                        Logger.GetSawmill("ArabicaAim").Info($"> DETECT: {gunName} | Type: HITSCAN | Src: {detectedInComp}");
                    else
                        Logger.GetSawmill("ArabicaAim").Info($"> DETECT: {gunName} | Type: BALLISTIC | Speed: {bulletSpeed}m/s ({ (speedDetected ? detectedInComp : "FALLBACK") })");
            ...
            {
                Logger.GetSawmill("ArabicaAim").Error($"Predict crash: {e.Message}");
            }
        }
