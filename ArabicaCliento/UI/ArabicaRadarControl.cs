...
public sealed class ArabicaRadarControl : Control
{
    ...
    private void UpdateMarkers(Vector2 playerPos, MapId mapId)
    {
        ...
        foreach (var uid in entities)
        {
            ...
            // 2. Поиск предметов
            if (ArabicaConfig.ItemSearchEnabled && !string.IsNullOrWhiteSpace(ArabicaConfig.ItemSearchQuery))
            {
                if (name.Contains(ArabicaConfig.ItemSearchQuery, StringComparison.OrdinalIgnoreCase))
                {
