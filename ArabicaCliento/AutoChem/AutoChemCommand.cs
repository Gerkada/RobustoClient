...
public sealed class AutoChemCommand : IConsoleCommand
{
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        var autoChem = entMan.System<ArabicaAutoChemSystem>();
        ...
        if (args.Length == 2 && args[0] == "calculate")
        {
            ...
            float best = ArabicaRecipeSolver.CalculateBestBatch(calcReagent, 100f);
            if (best <= 0)
            {
        ...
        {
            var results = ArabicaChemDatabase.SearchReagents(args[1]);
        ...
        var plan = ArabicaRecipeSolver.CreatePlan(reagent, amountInt);
        
        if (plan == null)
        {
            shell.WriteError($"[AutoChem] Рецепт '{reagent}' не найден!");
            var suggestions = ArabicaChemDatabase.SearchReagents(reagent);
