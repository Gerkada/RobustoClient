...
namespace ArabicaCliento.Systems.AutoChem;
...
public sealed class ArabicaAutoChemSystem : EntitySystem
{
    [Dependency] private readonly ArabicaWorldScanner _scanner = default!;
    [Dependency] private readonly ArabicaNetworkHands _networkHands = default!;
    ...
    public void StartJob(string reagent, FixedPoint2 amount)
    {
        ...
        _currentPlan = ArabicaRecipeSolver.CreatePlan(reagent, amount, capacity);
                    ...
                    if (_waitingForDispenseConfirm)
                    {
                        ...
                        if (reagentIncrease > 0.05f || productIncrease > 0.05f || volumeIncrease > 0.05f)
                        {
                            ...
                            if (productIncrease > 0.05f)
                            {
                                ...
                                {
                                    var recipe = ArabicaChemDatabase.GetRecipe(_targetProduct);
                                    if (recipe != null)
                                    {
