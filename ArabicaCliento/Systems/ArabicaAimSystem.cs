...
public class ArabicaAimSystem : EntitySystem
{
    ...
    [Dependency] private readonly ArabicaPredictionSystem _prediction = default!; 
    ...
    public override void Update(float frameTime)
    {
        ...
        if (!ArabicaConfig.RangedAimbotEnabled && !ArabicaConfig.ThrowAimbotEnabled)
        {
            _lockedTarget = null;
            return;
        }
        ...
        if (!inputMan.IsKeyDown(ArabicaConfig.TargetLockKey))
        {
            _lockedTarget = null;
            return;
        }
        ...
        if (!_lockedTarget.HasValue)
        {
            ...
            var target = GetClosestInScreenFov(inputMan.MouseScreenPosition, ArabicaConfig.AimFovPixels, new HashSet<EntityUid> { localPlayer.Value });
            if (target != null)
            {
    ...
    {
        if (!ArabicaConfig.RangedAimbotEnabled) return null;
        ...
        // ИСПОЛЬЗУЕМ ПИКСЕЛЬНЫЙ FOV
        var target = GetClosestInScreenFov(mousePos, ArabicaConfig.AimFovPixels, new HashSet<EntityUid> { localPlayer.Value });
    ...
    {
        if (!ArabicaConfig.ThrowAimbotEnabled) return null;

        var localPlayer = _player.LocalSession?.AttachedEntity;
        if (localPlayer == null) return null;

        // Определяем скорость броска динамически
        float throwSpeed = ArabicaConfig.DefaultThrowSpeed;
